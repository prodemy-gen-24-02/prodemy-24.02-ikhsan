...
const ProductDiscountCard = ({discounted}) => {  
  return (
      ...
      <div className="flex flex-row flex-wrap pt-2 gap-7">
        {discounted.map ((product) =>(
          <div key={product.id} className="flex flex-col w-[236px] group">
            ...
            <div>
                <p className="text-left px-1 font-bold text-md  text-red-700">{toRupiah(product.priceDiscount)}</p>
            </div>
            <div className="flex">
                <p className="text-left px-1 font-semibold text-md line-through text-gray-400">{toRupiah(product.price)}</p>

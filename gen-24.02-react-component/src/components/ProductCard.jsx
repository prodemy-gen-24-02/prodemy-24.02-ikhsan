...
const ProductCard = ({list}) => {
  ...
  return (
    <div className="flex flex-col">
      <h2 className="font-bold text-2xl pb-20">Produk Terlaris di Kategori Pria</h2>
      <div className="flex flex-row flex-wrap pt-2 gap-7">
        {list.map ((list) =>(
          <div key={list.id} className="flex flex-col w-[236px]">
            <div className="p-1 rounded-2xl shadow">
              <a href=""><img src={list.img} alt={list.name} /></a>
            </div>
            <div className="flex justify-between px-1 pt-1 items-end">
              <p className="font-bold text-lg">{list.merk}</p>
              <a href="">
            ...
            <div>
              <p className="text-left px-1">{list.name}</p>
            </div>
            <div>
              <p className="text-left px-1 font-semibold text-lg">{list.price}</p>
            </div>
            <Button>
              <p>Masukkan ke Tas</p>
            </Button>
